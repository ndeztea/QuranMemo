			...
			<div class="container-fluid">
					<div class="row">
						<div class="edit-profile-block" style="background-color: #FFFFFF;padding: 20px;min-height: 400px;margin-bottom: 20px">
							<strong>Total Santri : {{$countTotalUsers}}</strong>
							<div class="dropdown">
								  ...
								  <table class="table">
								    ...
								    @if(!empty($listUsers))
								    	...
								    	<tr>
									    	<td>{{$no++}}</td>
									    	<td>
									    		{{$user->name}} ({{$user->email}})<br><a href="https://api.whatsapp.com/send?phone={{$user->hp}}" target="_blank" class="label label-success"><i class="fa fa-whatsapp"></i> {{$user->hp}}</a> 
									    	...
									    	<td><input name="id_users[]" type="checkbox" value="{{$user->id}}"/></td>
									    	@endforeach
									    </tr>
								    @endif
								  </table>
